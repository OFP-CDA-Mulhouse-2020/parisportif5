...
final class UserUpdateControllerTest extends WebTestCase
{
    ...
    public function testUpdatePersonalDataFormValidDisplay(): void
    {
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data]'),
            "Il doit y avoir une et une seule balise form dans ce formulaire"
        );
        // Civilité
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] *[name*=civility]'),
            "Il doit y avoir un et un seul champ pour la civilité dans ce formulaire"
        );
        // Nom
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] input[name*=lastName]'),
            "Il doit y avoir un et un seul champ pour le nom dans ce formulaire"
        );
        // Prénom
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] input[name*=firstName]'),
            "Il doit y avoir un et un seul champ pour le prénom dans ce formulaire"
        );
        // Adresse
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] input[name*=Address]'),
            "Il doit y avoir un et un seul champ pour l'adresse dans ce formulaire"
        );
        // Ville
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] input[name*=City]'),
            "Il doit y avoir un et un seul champ pour la ville dans ce formulaire"
        );
        // Code postal
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] input[name*=Postcode]'),
            "Il doit y avoir un et un seul champ pour le code postal dans ce formulaire"
        );
        // Pays
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] *[name*=Country]'),
            "Il doit y avoir un et un seul champ pour le pays dans ce formulaire"
        );
        // Bouton submit
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_personal_data] *[type=submit]'),
    ...
    public function getUpdatePersonalDataForm(Crawler $crawler, array $formData): Form
    {
        ...
        $form['user_update_personal_data[lastName]'] = $formData['lastName'];
        $form['user_update_personal_data[firstName]'] = $formData['firstName'];
        $form['user_update_personal_data[billingAddress]'] = $formData['address'];
        $form['user_update_personal_data[billingCity]'] = $formData['city'];
        $form['user_update_personal_data[billingPostcode]'] = $formData['postcode'];
        $form['user_update_personal_data[billingCountry]'] = $formData['country'];
        return $form;
    }
    ...
    /*public function testUpdatePersonalDataFormCivilityLengthOverMax(): void
    {
        ...
        $this->assertSelectorTextContains(
            'form[name=user_update_personal_data]',
    ...
    public function testUpdateDocumentFormValidDisplay(): void
    {
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_document]'),
            "Il doit y avoir une et une seule balise form dans ce formulaire"
        );
        // Justificatif de domicile
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_document] *[name*=residenceProofFile]'),
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_document] *[name*=residenceProofAdd]'),
            "Il doit y avoir un et un seul champ pour le bouton d'ajout du justificatif de domicile dans ce formulaire"
        );
        // Justificatif d'identité
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_document] *[name*=identityDocumentFile]'),
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_document] *[name*=identityDocumentAdd]'),
        ...
        $this->assertCount(
            2,
            $crawler->filter('form[name=user_update_document] *[type=submit]'),
            "Il doit y avoir un et un seul bouton d'envoi dans ce formulaire"
        );
    }
    ...
    /*public function getUpdateDocumentForm(Crawler $crawler, array $formData): Form
    {
        ...
        $form['user_update_document[identityDocument]'] = $formData['identity'];
        return $form;
    }*/
    ...
    public function testUpdateIdentifierFormValidDisplay(): void
    {
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_identifier]'),
            "Il doit y avoir une et une seule balise form dans ce formulaire"
        );
        /*// Code de validation de l'email
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_identifier] input[name*=emailValidation]'),
        ...
        $this->assertCount(
            2,
            $crawler->filter(
                'form[name=user_update_identifier] input[id^=user_update_identifier_email_][type=email]'
            ),
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_identifier] *[type=submit]'),
            "Il doit y avoir un et un seul bouton d'envoi dans ce formulaire"
        );
    }
    ...
    public function getUpdateIdentifierForm(Crawler $crawler, array $formData): Form
    {
        ...
        $form['user_update_identifier[email][first]'] = $formData['newEmail1'];
        $form['user_update_identifier[email][second]'] = $formData['newEmail2'];
        return $form;
    }
    ...
    /*public function testUpdateIdentifierFormEmailEmpty(): void
    {
        ...
        // asserts
        $this->assertSelectorTextContains(
            'form[name=user_update_identifier]',
    ...
    public function testUpdatePasswordFormValidDisplay(): void
    {
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_password]'),
            "Il doit y avoir une et une seule balise form dans ce formulaire"
        );
        // Nom cacher pour validation
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_password] input[name*=lastName][type=hidden]'),
            "Il doit y avoir un et un seul champ cacher pour le nom dans ce formulaire"
        );
        // Prénom cacher pour validation
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_password] input[name*=firstName][type=hidden]'),
            "Il doit y avoir un et un seul champ cacher pour le prénom dans ce formulaire"
        );
        // Ancien mot de passe
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_password] input[name*=oldPassword][type=password]'),
        ...
        $this->assertCount(
            2,
            $crawler->filter(
                'form[name=user_update_password] input[id^=user_update_password_password_][type=password]'
            ),
        ...
        $this->assertCount(
            1,
            $crawler->filter('form[name=user_update_password] *[type=submit]'),
            "Il doit y avoir un et un seul bouton d'envoi dans ce formulaire"
        );
    }
    ...
    /*public function testUpdatePasswordFormPasswordWithName(): void
    {
        ...
        // asserts
        $this->assertSelectorTextContains(
            'form[name=user_update_password]',
